-- Script to create template tables in Supabase
...
SELECT 'packing_templates table created' AS status
WHERE EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'packing_templates');

SELECT 'meal_templates table created' AS status  
WHERE EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'meal_templates');