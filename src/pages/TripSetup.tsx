...
const TripSetup = () => {
  ...
  useEffect(() => {
    const loadTrips = async () => {
      const savedTrips = await tripService.getTrips();
  ...
  const getTripTypeInfo = (type: TripType) => {
    const info = {
      ...
      'day hike': {
        description: 'Single day hiking trip',
        tips: ['Pack a day pack', 'Check weather forecast', 'Bring plenty of water']
      }
    };
  ...
  const handleCreateTrip = async () => {
    ...
    try {
      ...
      try {
        const totalCampers = tripGroups.reduce((sum, group) => sum + group.size, 0) || 1;
        const start = new Date(startDate);
        const end = new Date(endDate);
        const tripDays = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
        const defaultPackingItems = getPackingTemplate(tripType, totalCampers, tripDays);
        await hybridDataService.savePackingItems(newTrip.id, defaultPackingItems);
      } catch (packingError) {
        console.error('Failed to create default packing list:', packingError);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <SEOHead
        ...
        url="https://planmyescape.ca/trip-setup"
      />
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 py-3 text-base border rounded-md dark:bg-gray-700 ${tripNameError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                      }`}
